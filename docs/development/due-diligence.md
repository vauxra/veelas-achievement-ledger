...
`achex-experimental` is a private/testing branch, not a normal Dalamud submission candidate. It intentionally explores achievement-progress behavior that the safer public/beta design avoids.
...
For `achex-experimental`, policy findings about direct progress requests or timers may be warnings if the README/Help clearly label the branch as experimental. Security, lifecycle, crash, privacy, and secret-handling findings remain blockers.
