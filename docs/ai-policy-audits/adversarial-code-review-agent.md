...
The safe default outside `achex-experimental` is:
...
If the diff moves away from that stance on public/beta/mainline branches, request changes. On `achex-experimental`, verify the risk is explicit and the experimental code remains isolated.
