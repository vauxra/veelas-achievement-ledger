...
def run_git_diff(base: str) -> str:
    result = subprocess.run(["git", "diff", base, "--", ":(exclude)*.md", ":(exclude)docs/**", ":(exclude).hermes/**", ":(exclude)scripts/audit-ai-policy.py", ":(exclude)scripts/adversarial-code-review.py"], check=False, text=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
