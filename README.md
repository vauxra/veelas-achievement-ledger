...
For a tree-style map of the `achex-experimental` architecture and runtime flow, see [`docs/achex-experimental-addon-flow.md`](docs/achex-experimental-addon-flow.md). It traces filenames and method/function calls down to the state, queues, native UI surfaces, hooks, reads, writes, and return values each flow touches.
...
Experimental build on `achex-experimental`. Current features:
